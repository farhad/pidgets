...
public class PidgetsViewPager extends ViewPager
{
    ...
    public PidgetsViewPager(Context context,AttributeSet attributeSet,int defStyle)
    {
        ...
        TypedArray typedArray = context.obtainStyledAttributes(attributeSet,R.styleable.PidgetsViewPager,defStyle,defStyle) ;
    ...
    private void initialize(Context context, TypedArray typedArray)
    {
        if(!isInEditMode())
        {
            swipeDurationMillis = typedArray.getInt(R.styleable.PidgetsViewPager_swipeDurationMillis, DEFAULT_SWIPE_DURATION) ;
            enableManualSwipe = typedArray.getBoolean(R.styleable.PidgetsViewPager_enableManualSwipe, true) ;
